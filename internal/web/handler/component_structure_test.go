...
// headerlessScreens are the routes a real page-head is not expected on, each
...
var headerlessScreens = map[string]string{
	...
	// `pagehead` is built for. Wave 2.5's acceptance (§7.9) verified these
	// against that frame as they stand — wrapping them in `pagehead` would
...
func checkShellScreensAssembleInOrder(t *testing.T, a *app) {
	...
	for _, path := range pageRoutes(t) {
		...
		if n := strings.Count(body, `<header class="page-head">`); n != 1 {
			t.Errorf("GET %s renders %d page-head headers; exactly one expected "+
