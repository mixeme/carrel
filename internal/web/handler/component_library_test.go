...
func TestComponentStylesheetAssembles(t *testing.T) {
	...
	// reason about.
	if head, bar := strings.Index(string(sheet.Body), "10-head.css"),
		strings.Index(string(sheet.Body), "20-bar.css"); head < 0 || bar < 0 || head > bar {
		t.Errorf("stylesheets are not concatenated in name order (head at %d, bar at %d)", head, bar)
	}
}
