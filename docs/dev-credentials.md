...
Каталога для вложений в переменных нет намеренно: integration-тесты создают собственный каталог со случайным именем на каждый прогон и удаляют его после, поэтому запись никогда не попадает туда, где уже что-то лежит. Каталог по умолчанию для вложений (§23.10) настраивается в UI и относится к ручной приёмке, а не к тестам.
