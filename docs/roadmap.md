...
**A collection added or removed on the server never reaches Carrel.** The collection list is discovered once, when the account is connected, and stored on it. **Refresh** clears the object cache and nothing else; **Test** re-runs discovery and throws the result away after saying «Connection test succeeded». Only editing the connection and saving it writes a fresh list. So a book deleted from the server by another client stays in the rail, keeps its link, is polled on every merged view and is counted in «N unavailable» — and a calendar created in another client is invisible until the person re-saves a connection they have no reason to touch. Found by deleting six collections from the test Baikal and watching them survive both buttons. What the fix should do is a decision, not an oversight: re-discovering on every Refresh puts a `PROPFIND` storm behind a button people press to see new records.

**The merged agenda does not fold a duplicate into one row (§15, mockup 7.2).** The frame draws the duty that lies in two calendars as **one** row whose source cell reads «2 sources»; the screen draws two rows. Nothing is missing from the detection — `/app/duplicates` finds exactly this pair at 60 points and offers to link it — only `collapseDuplicates` is called for the merged contacts directory and for nothing else (`find.go`, `req.Mode == modePeople`). Contacts collapse, and after linking the row carries «linked ×2» as drawn, so the mechanism is proven; the agenda, tasks and notes just do not use it.

**The merged tasks and notes views have no frame, and render as search results.** `/app/tasks` and `/app/notes` (Combined list / Combined notebook) are screens in their own right and the mockups draw neither — §7.3 and §7.4 are the per-collection lists, and those match the frames closely. The merged ones come out of `find.go` as `.m-row--find`: no `.m-tick` to complete a task, no `.m-tag` for categories, no percent column, and grouping by exact day instead of the frame's «Overdue / This week». This is the same miss as the task card that had no frame until stage 7 — a screen absent from the mockups does not get built from the library — and it needs the frame drawn first.

**Attachment settings are a screen with no frame; key escrow is a frame with no screen.** Both came out of О1 of the rebuild, where every template was addressed to the frame it is built from and the join was checked from both ends. `settings_attachments.html` answers on `/app/settings/attachments` and the mockups draw the rail entry for it in 7.10 and nothing else — no frame of the panel anywhere in the file. In the other direction, 7.10 draws **Настройки · депонирование ключа** as a screen at `/app/settings/escrow`, and there is no such route: the user's side of escrow is a `<fieldset>` inside `settings_account.html`, posting to `/app/escrow`. Neither is a defect a gate may settle on its own — §6 leaves that to a person, and the choice is the same one in both cases: draw the missing frame, or move the drawing to where the screen actually is. Until it is made, `TestScreenMatchesItsFrame` carries `settings_attachments` with the reason written into its entry instead of skipping it quietly, and С4 cannot rebuild that screen.
