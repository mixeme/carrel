...
**The merged tasks and notes views have no frame, and render as search results.** `/app/tasks` and `/app/notes` (All lists / All notebooks) are screens in their own right and the mockups draw neither — §7.3 and §7.4 are the per-collection lists, and those match the frames closely. The merged ones come out of `find.go` as `.m-row--find`: no `.m-tick` to complete a task, no `.m-tag` for categories, no percent column, and grouping by exact day instead of the frame's «Overdue / This week». This is the same miss as the task card that had no frame until stage 7 — a screen absent from the mockups does not get built from the library — and it needs the frame drawn first.

**Merged views are still called «All calendars», the mockups say «Combined calendar».** The rename (address book, list and notebook likewise) landed in the mockups as an explicitly mockup-only change: the checkboxes decide what is polled, so «all» is true only by accident, while «one calendar assembled from several» is true at any setting. `find.go` and the templates were left for later, and later has not come.
