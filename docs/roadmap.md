...
**Creating, renaming and deleting collections (§10.1).** *(done in wave 3.1)* Carrel can now create calendars and address books on a connected server (`MKCALENDAR`, extended `MKCOL`), rename and recolour them (`PROPPATCH`), and delete them with confirmation by typing the collection name. Entry points: rail-foot buttons on merged section views, **New collection** on each account card in Connections, and Rename/Delete on each collection row there. Reference rows for published links, backup jobs and davloom devices are wired for wave 4.
