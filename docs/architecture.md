...
The cache holds collection tags, path→ETag maps, object bodies and photo thumbnails, keyed so that a changed version is a miss rather than a stale hit: a body is stored under path *and* ETag together. Before reading a collection the server's `getctag` is compared; unchanged means the ETag map is reused without a request, changed means a fresh `PROPFIND`. A soft TTL covers servers that report no tag at all. Bodies are counted in bytes and evicted LRU *before* the map that names them, so memory pressure does not force a deep `PROPFIND` on the next visit. Thumbnails have a stricter separate ceiling because images are large and cheap to rebuild. A second, process-wide byte ceiling (bodies and thumbnails together) reaches LRU across every live session, so ten people with large address books cannot together exhaust the container; the data itself never leaves its session.
