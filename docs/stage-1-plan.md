...
**Статус:** **готово** (v0.1.0)  
**Источник:** [carrel-spec.md](carrel-spec.md) — §3, §4, §5, §13, §18, §19 этап 1, §21, §22, §24.3–§24.6  
**Следующий:** [stage-2-plan.md](stage-2-plan.md) · Индекс этапов: [stages.md](stages.md)

> Архив планирования с отмеченными чекбоксами. Открытые пункты приёмки: [stage-1-outstanding.md](stage-1-outstanding.md).

**Текущее состояние:** реализовано в коде; `go test ./...` проходит.
...
Этап 2 **не ждёт** полной ручной приёмки этапа 1. Достаточно:

| Проверка | Как |
|----------|-----|
| `go test ./...` без падений | CI / локально |
| `go test -race ./...` | dev-машина (закрыто в outstanding §1) |
| Bootstrap → login → admin panel | автотест `TestStageOneAcceptanceFlow` |

---

## Приёмка

Между этапами 1→2 — шлюз выше (автотесты). Ручной чеклист каркаса и Docker — [manual-acceptance.md](manual-acceptance.md) §1 (после всего объёма v1); промежуточный прогресс — [stage-1-outstanding.md](stage-1-outstanding.md).
