...
> Промежуточный трекер этапа 1. **Финальная ручная приёмка** всего v1 — [manual-acceptance.md](manual-acceptance.md) (после этапа 7).

Этап 1 **реализован в коде** и выпущен как **v0.1.0**. Ниже — что ещё не закрыто в промежуточном трекере: прогоны в окружении и намеренно отложенное на этап 2+.
...
| `go test ./...` | проходит |
| `go test -race ./...` | **пройден** на dev-машине (Windows, MSYS2 UCRT64 gcc) |
| Ручная приёмка локально (`go run`, `dev/data`) | **частично** — см. §2a |
...
Требует установленный C-компилятор (`gcc` на Linux, Xcode CLT на macOS; на Windows — MSYS2 UCRT64 в `PATH`). На dev-машине прогон успешен:
...
**Закрытие:** один успешный прогон без падений; при обнаружении гонок — исправить код. — **закрыто** на dev-машине.

---

## 2a. Локальная ручная приёмка (`go run`, `dev/data`)

Проверено вручную через локальный сервер (не Docker). Инструкции: `dev/local-testing.md` (не в git).

### Первый запуск и администратор (A1, без healthcheck)

- [x] `http://127.0.0.1:8080/` перенаправляет на `/setup` (на пустом томе)
- [x] Форма создаёт первого администратора и сразу открывает панель
- [x] Повторное открытие `/setup` ведёт на `/login`, второго администратора завести нельзя
- [ ] `docker compose ps` / healthcheck — только в Docker (§2)

### About (A2, частично)

- [x] `/about` открывается без входа
- [ ] Версия и коммит совпадают с ldflags сборки (`go run` даёт `commit: unknown`)
- [x] Ссылка на исходники ведёт на https://github.com/mixeme/carrel, лицензия AGPL-3.0-or-later

### Приглашение по ссылке без SMTP (A8)

- [x] SMTP не настроен: админ создаёт **ссылку** (сценарий «By link»), копирует и передаёт
- [x] В таблице приглашений доставка `link`, колонка Mail — `—`
- [x] По ссылке пользователь **сам выбирает логин**, задаёт пароль и попадает внутрь
- [x] Повторный переход по той же ссылке отдаёт «недействительно»

### Админка и пользователи

- [x] Disable пользователя сразу рвёт его активную сессию (не дожидаясь следующего запроса)
- [ ] Сброс пароля админом предупреждает о потере данных до подтверждения
- [ ] После сброса подключения недоступны
- [ ] Удаление последнего администратора невозможно
- [ ] На экране входа нет вводящего в заблуждение «forgot password»

### Ещё не проверено локально

- [ ] Приглашение **по email** (сценарий «By email», нужен SMTP)
- [ ] A3–A7, A5 (логи), A9 (`THIRD_PARTY.md` глазами)
...
### A8. Приглашение по ссылке без SMTP (сквозной проход руками)

> Локально проверено — см. §2a. Сценарии «ссылка» и «email» разделены; здесь — только ссылка.
...
Этап 1 считается **полностью принятым в коде**, когда:

1. `CGO_ENABLED=1 go test -race ./...` проходит без падений. — **да** (dev-машина)
2. Автотесты §21 для каркаса проходят (`go test ./...`).

Формальная ручная приёмка (Docker A1–A9 и остальное) перенесена в общий чеклист [manual-acceptance.md](manual-acceptance.md) §1 и выполняется **по завершении всего объёма v1**, не блокируя этапы 2+.

После финальной приёмки v1 этот файл можно удалить или свести к «закрыто».
