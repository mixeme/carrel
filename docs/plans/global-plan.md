...
| **v1.1.1** | Компоненты, которые кадры 7.1–7.5 показывают, а спека не описывала: бар над списком, `⋯`, иконки файлов, Import/Export на объединённых экранах | 2.6 |
...
flowchart LR
  ...
  W25 --> W26["Волна 2.6 · компоненты макета"]
...
**Вывод для тега.** Волна 2.5 в границах, которые она сама себе поставила («только вёрстка, разметка шаблонов и `carrel.css`»), закрыта. Оставшееся к вёрстке не относится — см. ниже.
...
## 7-бис. Волна 2.6 — компоненты, которых требует макет

Отдельная волна, а не хвост 2.5, потому что это **возможности**, и порядок внутри них тот же, что везде: модель → служба → обработчик → шаблон. Стоит **после** волны 3 по значимости, но дешевле её, поэтому идёт впереди по календарю там, где не конфликтует.

| | Шаг | Решение по правилу 3 | Куда |
|---|---|---|---|
| **2.6.1** | **Панель `.m-bar` на объединённых экранах.** Контейнер `.page-bar` уже совпадает с `.m-bar` до пикселя и стоит на поколлекционных экранах; на «All …» его нет | **Делаем.** Пустой бар не нужен — шаг имеет смысл только вместе с 2.6.2 | 2.6 |
| **2.6.2** | **Наполнение бара: фильтр по загруженному, сортировка, счётчик.** Фильтр — по уже пришедшим записям, без второго запроса к серверам (§16 запрещает лишний веер). Сортировка: контакты A–Z / по изменению, заметки Newest / Oldest / Title, задачи By due / By priority | **Делаем** для контактов, задач, заметок, событий. **Неприменимо** для файлов: там свой бар выделения, он уже есть | 2.6 |
| **2.6.3** | **Compact rows в баре.** Плотность уже реализована и живёт в настройках Appearance; макет ставит переключатель в бар | **Делаем** — перенос точки входа, не новая механика | 2.6 |
| **2.6.4** | **Import / Export на объединённых экранах.** Поколлекционно и то и другое есть; на «All …» нужен выбор коллекции-назначения | **Делаем** для контактов, событий, заметок. **Неприменимо** для задач (импорта задач нет и не обещано) и файлов | 2.6 |
| **2.6.5** | **Меню `⋯` и его иконка `i-dots`.** Есть на каждом кадре; сегодня действия второго ряда некуда положить | **Делаем.** Состав меню — то, что вытеснено из `.m-acts` | 2.6 |
| **2.6.6** | **Иконки файлового экрана** — `i-grid`, `i-img`, `i-doc`, `i-pencil`, `i-move`, `i-trash`, `i-panel`, `i-link`, `i-server`. Кнопки существуют текстом | **Делаем** — чистый перенос, единственный шаг волны, который правда является вёрсткой | 2.6 |
| **2.6.7** | **Колонки строки контакта на поколлекционном экране** — 5 против 7 в макете: нет колонки телефона и колонки источника | **Делаем.** Источник на поколлекционном экране — **неприменимо** (коллекция одна); колонка телефона — делаем | 2.6 |

**Счётчики — отдельно, и они не в этой волне.** Макет пишет число у каждого раздела в рейле («Contacts 312») и у каждой коллекции. Чтобы его показать, нужно спросить **каждую** коллекцию, включая неотмеченные, — а §16 называет веерный запрос самым дорогим местом архитектуры и требует не делать лишних. Это не перенос вёрстки и не мелкая функция: это решение о том, откуда берутся числа (кэш ETag? отдельный дешёвый `PROPFIND`? только для уже опрошенных?). **Сознательно позже:** записать в roadmap как открытый вопрос и решать вместе с §12 (кэш, волна 3.2), где потолки и вытеснение всё равно пересматриваются.

**Подвал рейла разбирается по частям, и большая его часть уже в плане:**

| Строка подвала | Куда относится |
|---|---|
| New address book / New calendar | **Волна 3.1** (§10.1, коллекции) — уже запланировано |
| Subscribe to a feed | **Волна 4.3** (§23.4, подписки) — уже запланировано |
| Connect account | **2.6** — экран существует, нужна только вторая дверь в рейле |
| Порог совпадений у дубликатов | **Волна 3** — это **долг спеки**, а не добавка макета: §15 говорит «порог настраиваемый, это настройка, которую меняют», а сегодня он только в `config.json` и в переменной окружения. Добавить как **шаг 3.7** |
...
| **2.6 — компоненты макета** | **2.6.1–2.6.7** | **☐ · вскрыто приёмкой (§7.8); счётчики вынесены в roadmap** |
