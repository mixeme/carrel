...
### 7.8 Противоречие, которое приёмка вскрыла *(снято правилом 1 в нынешней редакции)*
...
**Вторая находка того же разбора:** этих компонентов нет **ни в спеке, ни в roadmap** — они существуют только в макете. Разбор августа 2026 читал это как «макет тихо ввёл требования, которых не проходил ни один документ», и в этом чтении отсутствующий фильтр выглядел добавкой, которую нельзя брать.

Чтение было неверным, и правило 1 в нынешней редакции его отменяет: макет рисовался с пониманием того, как экраном пользуются, значит **пропуск в спеке, а не выдумка макета**. Проверено на трёх пунктах, каждый из которых оказался невыполненным обещанием другого документа: повторная проверка подключения (§6 обещает диагностику отказа — но только при добавлении аккаунта), сеансы в учётной записи (§24 говорит о завершении сеансов; админка это умеет, человек про свои — нет), порог дубликатов (§15 дословно: «настройка, которую меняют»). Каждому по-прежнему нужен ответ по правилу 3 — но ответ на вопрос «когда и в какой волне», а не «делать ли вообще».
...
Отдельная волна, а не хвост 2.5: это **возможности**, и порядок внутри каждой обычный, снизу вверх — модель → служба → обработчик → шаблон. Волна отвечает на два требования сразу: экран должен совпасть с кадром **и работать**, то есть каждый добавленный контрол обязан что-то делать на живых данных, а не изображать.
...
Работа мелкая: значение уже читается из конфигурации, нужно только положить его в профиль пользователя рядом с прочими предпочтениями и дать контрол. Хранение — там же, где решения о дубликатах (§15, «хранение решений»), тем же DEK.
