...
|---|---|---|---|
| **2.6.A1** | `{{template "pagehead"}}` — заголовок, подпись, полоска коллекции, набор действий, `⋯` | 30 копий сводятся в одну; десять шаблонов со старой шапкой переводятся | Тест: ни один шаблон не содержит `page-head` с `<h1>` без класса |
...
|---|---|---|---|
| **2.6.B1** | **Фильтр по загруженному.** Поле в панели, сужает то, что уже пришло | Целиком в браузере: строки скрываются по подстроке, счётчик пересчитывается. Ни одного запроса. Без JS поле не показывается — фильтровать нечем | Контакты, задачи, заметки, события, файлы — **делаем** |
...
1. `go test ./...` зелёный, включая новые тесты на разметку из 2.6.A.
...
| **2.6 — довести экраны до кадров** | **2.6.A1–A4, B1–B6, C1–C7, D1–D4** | **☐ · полная сверка проведена (§7.9); библиотека компонентов первой; счётчики в рейле вынесены в roadmap** |
